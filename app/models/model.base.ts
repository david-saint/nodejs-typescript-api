...
import Paginator from './paginator.base';
...
export default class AbstractModel extends Model {
   ...
   */
  static async find(primaryKey: any, options: Record<string, any> = {}): Promise<Model> {
    // find the model
   ...
   * @return {[type]}         [description]
   */
  static async paginate(request: Request, options = {}) {
    // get the current page
    const page = Paginator.resolveCurrentPage(request);
    // @ts-ignore get the number of results to display per page
